...
Module.afterInit(function() {
  ...
  let g_obj_get_designations_callback = Module.addFunction(function(o, u, v) {
    // Copy the string NOW: many designations (cultural names, "CAT value"
    // forms) are formatted in C stack buffers that are reused/clobbered as
    // the enumeration continues — a pointer read after the call returns
    // decodes whatever was written there last.
    g_ret.push(Module.UTF8ToString(v));
  }, 'viii');
  ...
  SweObj.prototype.designations = function() {
    ...
    let ret = g_ret;  // already strings (copied in the callback)
    // Remove duplicates.
  ...
  /*
   * Function: namesLocalized
   *
   * Like designations(), but resolved for DISPLAY in the current language,
   * uniformly for every object type. Item 0 is always the best common name:
   * the skyculture common name when the object has one (translated, e.g.
   * 勾陈一 for Polaris under zh), else the object's proper name translated
   * through the sky catalog (e.g. "NAME 火星" for Mars), else the raw
   * designation. The remaining items are the other designations.
   */
  SweObj.prototype.namesLocalized = function() {
    g_ret = [];
    Module._skycultures_get_designations(this.v, 0,
   ...
   *
   * The single localized display title for this object — what the host app
   * should show as the object's name. Equals namesLocalized()[0] prettified
   * (the "NAME " prefix stripped, bayer/catalog forms formatted).
   */
  SweObj.prototype.displayName = function() {
    const names = this.namesLocalized();
    if (!names.length) return '?';
    return Module.designationCleanup(names[0], 4);
  };
  ...
  SweObj.prototype.culturalDesignations = function() {
    // Keep the C pointer in its own variable: freeing the decoded JS string
    // (which coerces to 0) would leave the calloc'd JSON buffer leaked.
    const cptr = Module._skycultures_get_cultural_names_json(this.v);
    const ret = Module.UTF8ToString(cptr);
    Module._free(cptr);
  ...
  *    obj.destroy() on each of them.
  ...
  SweObj.prototype.listObjs = function(obs, maxMag, filter) {
    ...
    // Snapshot the pointers before constructing any SweObj: the SweObj
    // constructor reassigns the shared g_ret while enumerating attrs/children,
    // which would otherwise clobber this loop (and return at most one object).
    const ptrs = g_ret;
  ...
  SweObj.prototype._call = function(attr, arg) {
    // Any call WITH an argument is a mutation (attribute set or method):
    // wake the adaptive render loop (canvas.js) so host-app driven changes
    // (selection, time set, visibility toggles...) show at full frame rate
    // even though they arrive without any DOM input event.
    if (arg !== undefined && arg !== null && Module._apiActivity)
      Module._apiActivity()
  ...
  function stringToC(str) {
    ...
    // writeAsciiToMemory was removed in modern emscripten; stringToUTF8 writes
    // the string plus a null terminator into the (size + 1)-byte buffer.
    Module.stringToUTF8(str, ptr, size + 1);
    return ptr;
  }
