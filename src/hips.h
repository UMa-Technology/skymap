...
struct hips {
    ...
    // If set, skip the ~1 sec per-tile download delay (HIPS_NO_DELAY) for this
    // survey.  Only safe for bundled/same-origin surveys: it removes the
    // anti-hammer protection that ASSET_DELAY provides for remote servers.
    bool no_delay;
};
...
/*
 * Function: hips_set_no_delay
 * Enable/disable the per-tile download delay for a survey.
 *
 * By default hips tiles are downloaded with a small delay (HIPS_NO_DELAY unset)
 * to avoid hammering remote servers while the view is still moving.  For a
 * bundled/same-origin survey there is no server to protect, and the delay only
 * adds latency (e.g. a visible blank while the DSS layer fades in), so it can
 * be turned off.
 *
 * Parameters:
 *   hips  - A hips survey.
 *   value - true to skip the download delay.
 */
void hips_set_no_delay(hips_t *hips, bool value);
...
/*
 * Function: hips_get_render_order_clamped
 * Same as hips_get_render_order, clamped into the survey's available orders
 * exactly like hips_render will.  Use this when deriving a split order, so
 * the split count stays bounded once the fov order exceeds the survey's max
 * order (an unclamped base makes 1 << (split_order - render_order) explode
 * at deep zoom).
 */
int hips_get_render_order_clamped(const hips_t *hips, const painter_t *painter);
...
/*
 * Function: hips_prefetch
 * Warm the tile cache for the tiles a hips_render() at the current view would
 * need, WITHOUT painting anything.
 *
 * Used to preload a survey while it is still fully transparent (e.g. the DSS
 * layer before it fades in), so that by the time it becomes visible the tiles
 * are already downloaded and decoded and there is no blank flash.
 *
 * Parameters:
 *   hips    - A hips survey.
 *   painter - The painter that would be used to render (for the clip/fov).
 */
void hips_prefetch(hips_t *hips, const painter_t *painter);
