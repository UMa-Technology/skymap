...
static void core_set_default(void)
{
    ...
    core->star_exposure_scale = 2; // = exposure_scale: no change unless overridden
    core->star_glow_scale = 0;    // No halo (the old hard-coded glow was 0.20).
...
{
    bool atm_visible, animating, adapting;
    double lwmax, now, dt;
    int r;
    obj_t *atm, *module;
    ...
    // Adaptation still easing => the image is visibly changing frame to frame,
    // so report it as animation below (2% luminance step threshold).
    adapting = fabs(log(core->lwmax / core->tonemapper.lwmax)) > 0.02;
    ...
    DL_FOREACH(core->obj.children, module) {
        if (module->klass->update) {
            ...
            if (r > 0) animating = true;
        }
    }

    // Report whether anything is still visibly animating, so the JS render
    // loop can drop to a very low frame rate on a fully static sky (mobile
    // heat).  Faders are collected through a global flag (fader.c) so every
    // module is covered without changing each update function.  Deliberately
    // NOT included: target.lock tracking — it only drifts at sidereal rate,
    // which the render loop models as accumulated pixel motion instead.
    animating |= fader_pop_global_activity();
    animating |= adapting;
    animating |= core->target.src_time != 0;
    animating |= core->fov_animation.src_time != 0;
    animating |= core->time_animation.src_time != 0;
    animating |= core->zoom != 0;
    // Data still loading (survey tiles, landscapes...): keep frames coming so
    // it appears as soon as it is ready.  Only counts bars actively making
    // progress — errored/stalled bars must not pin the frame rate forever.
    animating |= progressbar_count_active() > 0;
        ...
        double mag, double *radius, double *luminance)
{
    ...
    // Optional separate star/point exposure. tonemapper_map is linear in the
    // tonemapper exposure (== exposure_scale), so scaling by the ratio makes
    // points behave as if rendered at star_exposure_scale, dimming the star
    // field without touching the sky/DSS/atmosphere.
    if (core->exposure_scale > 0 &&
            core->star_exposure_scale != core->exposure_scale)
        ld *= core->star_exposure_scale / core->exposure_scale;
...
/*
 * Cap on the final point luminance, i.e. on the vertex alpha of every point
 * source (stars, but also planets/satellites/comets rendered as points).
 *
 * Points are drawn with additive blending (GL_SRC_ALPHA, GL_ONE), so on a
 * black sky the screen value is simply rgb * luminance: a luminance of 1.0
 * puts a white star at full 255 against a 0 background, the maximum contrast
 * possible, which reads as glary on high-brightness mobile screens.
 *
 * The radius is computed before the cap is applied, so the bright/faint size
 * hierarchy is fully preserved; only the peak brightness is lowered.
 */
#define MAX_POINT_LUMINANCE 0.8
 ...
 *   luminance - Output luminance, gamma corrected, from 0 to
 *               MAX_POINT_LUMINANCE.  Ignored if set to NULL.
 */
bool core_get_point_for_mag(double mag, double *radius, double *luminance)
{
    ...
    *radius = r;
    // Note: the cap here is the only one that matters. The clamp in
    // core_get_point_for_mag_ runs before the gamma correction above, and
    // gamma is monotonic, so lowering it too would change nothing.
    if (luminance) *luminance = clamp(ld, 0, MAX_POINT_LUMINANCE);
    return true;
}

void core_get_selection_label_color(double out[4])
{
    if (core->nightmode)
        vec4_set(out, 1, 1, 1, 1);
    else
        vec4_set(out, 0.95686, 0.50588, 0.13725, 1); // #F48123
}
...
int core_render(double win_w, double win_h, double pixel_scale)
{
    ...
    fps_prev_avg = core->fps.avg;
    fps_tick(&core->fps, sys_get_unix_time());
    // Only notify when the averaged value actually changes (~once a second):
    // module_changed rebuilds the whole core SweObj + a Vuex commit on the JS
    // side, so firing it every frame is pure per-frame waste.
    if (core->fps.avg != fps_prev_avg)
    ...
    painter_t painter = {
        ...
        .points_halo = 7.0,
        .points_glow = core->star_glow_scale,
...
static obj_klass_t core_klass = {
    ...
    .attributes = (attribute_t[]) {
        ...
        PROPERTY(star_exposure_scale, TYPE_FLOAT,
                 MEMBER(core_t, star_exposure_scale)),
        PROPERTY(star_glow_scale, TYPE_FLOAT,
                 MEMBER(core_t, star_glow_scale)),
        ...
        PROPERTY(nightmode, TYPE_BOOL, MEMBER(core_t, nightmode)),
