...
struct item
{
    ...
    union {
        ...
        struct {
            // double (not float like the other item params): these are compared
            // against the painter's double values to decide batch reuse, and a
            // float copy of a non-representable value (e.g. glow 0.20) never
            // compares equal, which silently disables points batching.
            double halo;
            double glow;
        } points;
                      ...
                      int n, const point_t *points)
{
    ...
    if (item && item->points.halo != painter->points_halo)
        item = NULL;
    if (item && item->points.glow != painter->points_glow)
        item = NULL;
    ...
    if (!item) {
        ...
        item->points.glow = painter->points_glow;
                 ...
                 double bounds[4])
{
    ...
    // Clamp every on-canvas text label to a uniform size range: max 15px,
    // min 11px, sizes in between unchanged. This is the single choke point for
    // both text rendering and bounds computation, so a label's layout box stays
    // consistent with the size it is drawn at.
    size = fmax(11.0, fmin(15.0, size));

    // Prevent overflow in nvg. This happens for labels of objects projected far
    // off-screen (e.g. behind the viewer); it is handled by skipping the draw, so
    // no warning is logged (it would otherwise spam the console every frame).
...
static void item_points_render(renderer_t *rend, const item_t *item)
{
    ...
    gl_update_uniform(shader, "u_glow", item->points.glow);
...
static void item_text_render(renderer_t *rend, const item_t *item)
{
    ...
    // Wrap-guard margin: nvgTextBox word-wraps, and a box width close to the
    // measured text width can push the last word (e.g. "HIP 101221") onto a
    // second line whenever the draw-time line width exceeds the measured width
    // by a hair -- sub-pixel rounding, or font-metric differences between build
    // targets (the +1px slack was enough for the web build but marginal
    // elsewhere). Use a few px of slack so single-line labels never wrap; this
    // only widens the wrap box, shifting a center-aligned label right by ~half
    // the slack (negligible). Explicit '\n' still wraps as intended.
    w = bounds[2] - bounds[0] + 4.0;
    ...
    // properly aligned.
    float tx = bounds[0], ty = roundf(bounds[1] - descender);

    // Dark halo for legibility: sky labels sit over wildly varying
    // backgrounds (dark sky vs bright Milky Way / DSS nebulosity), where no
    // single text colour stays readable. Draw a blurred near-black copy under
    // the glyphs first (two passes to deepen it), then the real coloured text
    // on top. Halo alpha tracks the label's own (possibly faded) alpha so
    // fade-in/out and the constellation focus effect stay coherent. Blur is in
    // CSS px (nvgBeginFrame already divides by scale), so it is DPR-consistent.
    if (item->color[3] > 0.004f) {
        float blur = fminf(fmaxf(item->text.size * 0.15f, 1.5f), 3.0f);
        unsigned char ha = (unsigned char)(0.72f * 255 * item->color[3]);
        nvgFontBlur(rend->vg, blur);
        nvgFillColor(rend->vg, nvgRGBA(0, 0, 0, ha));
        nvgTextBox(rend->vg, tx, ty, w, item->text.text, NULL);
        nvgTextBox(rend->vg, tx, ty, w, item->text.text, NULL);
        nvgFontBlur(rend->vg, 0.0f);
        nvgFillColor(rend->vg, nvgRGBA(item->color[0] * 255,
                                       item->color[1] * 255,
                                       item->color[2] * 255,
                                       item->color[3] * 255));
                 ...
                 const double (*line)[3], const double (*win)[3], int size)
{
    ...
    int i, ofs;
    float color[4];
    double depth, line_width;
    item_t *item;
    const int SIZE = 2048;

    if (size <= 1) return;
    vec4_to_float(painter->color, color);
    // A glowing line needs a wide quad to fit the ~5px glow radius. A plain
    // line (glow == 0, e.g. coordinate grids) only needs its width plus the
    // anti-aliasing falloff, which greatly cuts fragment overdraw.
    line_width = painter->lines.glow > 0 ?
                 fmax(10, painter->lines.width + 2) : painter->lines.width + 2;
    mesh = line_to_mesh(line, win, size, line_width);
    ...
    if (item && item->lines.width != painter->lines.width)
        item = NULL;
    // glow is a per-item uniform and changes the baked quad width, so lines
    // with different glow must not share a draw item (grids/constellations now
    // use glow=0 while the default painter uses 0.2).
    if (item && item->lines.glow != painter->lines.glow)
        item = NULL;
    if (item && item->flags != painter->flags)
        item = NULL;
                   ...
                   int size)
{
    int id;
    int font;
    // The renderer is normally created lazily on the first frame, but the
    // JS side can call setFont before that (the GL context already exists
    // by then); create it here too or rend->vg below is a NULL deref.
    if (!rend && !core->rend)
