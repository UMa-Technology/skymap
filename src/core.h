...
struct core
{
    ...
    // Night mode: annotation modules (DSO/star markers + labels) paint white so
    // the #FF6C20 CSS multiply overlay renders them as #FF6C20. Set from the UI.
    bool            nightmode;
    ...
    // Separate exposure applied only to point sources (stars, and planets/
    // objects rendered as points), so the star field can be dimmed
    // independently of the sky/DSS/Milky Way/atmosphere (which use
    // exposure_scale). Equal to exposure_scale by default (no change).
    double          star_exposure_scale;
    // Strength of the additive halo/glow drawn around bright point sources
    // (the shader's u_glow). Higher makes bright stars more glary. Default 0
    // (no halo; the shader's historical hard-coded glow was 0.20). Tunable at
    // runtime via core.star_glow_scale.
    double          star_glow_scale;
...
/*
 * Function: core_get_selection_label_color
 * Fill the label colour used for the currently-selected object.
 *
 * Returns #F48123 (matching the selection marker), except in night mode where
 * it returns white so the fullscreen #FF6C20 multiply overlay yields #FF6C20
 * (drawing #F48123 there would be multiplied twice into a dark, unreadable
 * red). Shared by every module that labels a selectable object so the scheme
 * cannot drift per module.
 *
 * Parameters:
 *   out - Output RGBA colour (alpha set to 1).
 */
