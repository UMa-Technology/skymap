...
typedef struct constellation {
    ...
    // Fades this constellation's lines + name out when ANOTHER constellation is
    // selected (focus effect); 1 = shown, 0 = hidden. Driven in
    // constellations_update, applied in render_lines / render_label.
    fader_t     focus_fader;
    // Eases THIS constellation between its idle look and its selected highlight
    // (lines/name up to 70%, forced visible); 1 = selected, 0 = not.
    fader_t     sel_fader;
...
static int constellation_init(obj_t *obj, json_value *args)
{
    ...
    fader_init2(&cons->focus_fader, true, 0.5); // Shown until another is picked.
    fader_init2(&cons->sel_fader, false, 0.5);  // Highlight eases in on select.
                         ...
                         bool selected)
{
    ...
    // Selection boundary eases in/out via sel_fader; a bounds-layer boundary
    // keeps its own layer + on-screen fade.
    const double sel = con->sel_fader.value;
    painter.color[3] *= fmax(cons->bounds_visible.value * con->visible.value,
                             sel);
    ...
    if (sel > 0.0) {
        // Selected constellation boundary: white, 50%, crisp (no halo).
        vec4_set(painter.color, 1.0, 1.0, 1.0, 0.5 * painter.color[3]);
                        ...
                        bool selected)
{
    ...
    const double sel = con->sel_fader.value;
    const double layer = cons->lines_visible.value * con->visible.value;
    // Effective visibility: the lines layer, or the selection highlight, which
    // forces the figure on even with the layer off. Both fade, so selecting /
    // deselecting eases the figure in and out.
    visible = fmax(layer, sel);
    painter.lines.width *= 1.0 + sel; // x1 idle -> x2 fully selected
    if (painter.color[3] == 0.0 || visible == 0.0) return 0;
    assert(vec3_norm2(con->lines.cap) > 0);
    if (painter_is_cap_clipped(&painter, FRAME_ICRF, con->lines.cap))
        return 0;

    // White, 30% at rest easing up to 50% when selected.
    vec4_set(lines_color, 1.0, 1.0, 1.0, fmax(0.3 * layer, 0.5 * sel));
    vec4_emul(lines_color, painter.color, painter.color);
    painter.color[3] *= con->focus_fader.value; // Dim out while another is picked.
    painter.lines.glow = 0; // Constellation figure lines: crisp, no halo.
...
// A few constellations carry no illustration of their own and are drawn by a
// neighbour's shared image (Argo Navis: Carina hosts Puppis & Vela; Ophiuchus
// hosts Serpens — same special cases as constellation_is_pointed). If `con` is
// such a host, return the combined selection highlight (0..1, clamped) of its
// hosted children; 0 if con is not a host or no hosted child is highlighted.
// Keyed on the children's sel_fader (not core->selection) so the shared art
// eases smoothly in AND out with the child's selection (no blink when the
// selection is cleared); SUMMED (not max'd) so switching directly between two
// children of the same host — Puppis<->Vela — holds the art at full highlight
// instead of dipping to idle at the cross-fade midpoint. Used so selecting
// Puppis/Vela/Serpens keeps the shared art lit instead of dimming it away.
static double image_host_child_sel(const constellation_t *con)
{
    obj_t *module = con->obj.parent;
    constellation_t *c;
    double sum = 0.0;
    const bool is_car = strcmp(con->info.iau, "Car") == 0;
    const bool is_oph = strcmp(con->info.iau, "Oph") == 0;
    if (!is_car && !is_oph) return 0.0;
    MODULE_ITER(module, c, "constellation") {
        const bool match = is_car ?
            (strncasecmp(c->info.iau, "Pup", 3) == 0 ||
             strncasecmp(c->info.iau, "Vel", 3) == 0) :
            (strncasecmp(c->info.iau, "Ser", 3) == 0);
        if (match) sum += c->sel_fader.value;
    }
    return fmin(1.0, sum);
}
                      ...
                      bool selected)
{
    ...
    // Mirror the constellation-lines focus effect: the images-layer opacity
    // (0.3) eases up to the selection highlight (0.6), a selection forces this
    // image on even with the layer off, and every OTHER constellation's image
    // dims out (focus_fader) while one is selected. Exception: if this image is
    // the shared art of the selected constellation (Carina<-Puppis/Vela,
    // Ophiuchus<-Serpens), keep it lit and ease it on the child's sel_fader.
    const double host_sel = image_host_child_sel(con);
    const bool hosted = host_sel > 0.0;
    const double sel = hosted ? host_sel : con->sel_fader.value;
    const double focus = hosted ? 1.0 : con->focus_fader.value;
    const double layer = cons->images_visible.value * con->visible.value;
    double figure = fmax(0.3 * layer, 0.6 * sel);
    figure *= focus;
    if (figure == 0.0) return 0;
    ...
    painter.color[3] *= figure * con->image_loaded_fader.value;
                        ...
                        bool selected)
{
    ...
    assert(con->first_update_complete);
    const double sel = con->sel_fader.value;
    // Idle label opacity follows the labels layer + on-screen fade; the
    // selection eases it up and forces it on even when the layer is off.
    painter.color[3] *= fmax(cons->labels_visible.value * con->visible.value,
                             sel);
    ...
    // 50% at rest easing up to 70% when selected.
    vec4_set(names_color, 1.0, 1.0, 1.0, (0.5 + 0.2 * sel) * painter.color[3]);
    names_color[3] *= con->focus_fader.value; // Dim out while another is picked.
...
static int constellation_render(obj_t *obj, const painter_t *_painter)
{
    ...
    // Early exit if nothing should be rendered. A host of a selected child
    // (Carina/Ophiuchus) keeps rendering so its shared art shows even with every
    // layer off — same as a selected constellation forces its own image on.
    if (    !selected
            && con->sel_fader.value == 0.0
            && cons->lines_visible.value == 0.0
            && cons->labels_visible.value == 0.0
            && cons->images_visible.value == 0.0
            && cons->bounds_visible.value == 0.0
            && image_host_child_sel(con) == 0.0)
        return 0;

    con->visible.target = constellation_is_visible(con, &painter);
    if (con->visible.value == 0.0) return 0;

    // Focus fade (other constellations dim out while one is selected) is applied
    // inside render_lines / render_label via con->focus_fader.
...
// True while any constellation's selection highlight is still fading out, so
// the module keeps updating/rendering long enough to finish the fade even when
// every constellation layer is switched off.
static bool any_selection_fade(obj_t *obj)
{
    constellation_t *con;
    MODULE_ITER(obj, con, "constellation")
        if (con->sel_fader.value != 0.0) return true;
    return false;
}
...
static int constellations_update(obj_t *obj, double dt)
{
    ...
    if (cons->lines_visible.value == 0.0 &&
        ...
        cons->bounds_visible.value == 0.0 &&
        cons->labels_visible.value == 0.0 &&
        (!core->selection || core->selection->parent != obj) &&
        !any_selection_fade(obj)) return 0;

    // Focus effect: when the selection is a constellation, every OTHER
    // constellation fades its lines + name out; all fade back in on deselect.
    const bool a_con_selected = core->selection &&
                                core->selection->parent == obj;
    MODULE_ITER(obj, con, "constellation") {
        const bool this_selected = &con->obj == core->selection;
        con->focus_fader.target = !(a_con_selected && !this_selected);
        con->sel_fader.target = this_selected;
        fader_update(&con->image_loaded_fader, dt);
        fader_update(&con->visible, dt);
        fader_update(&con->focus_fader, dt);
        fader_update(&con->sel_fader, dt);
    }
    return 0;
}
