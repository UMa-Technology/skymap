                             ...
                             double color[3])
{
    ...
    if (selected) {
        // Selected label: #F48123 to match the selection marker (readable).
    ...
    // Star names render at the same fixed 13px bold as DSO labels.
    labels_add_3d(buf, frame, pos, true,
                 radius, FONT_SIZE_BASE - 2, label_color, 0, 0,
                 effects | TEXT_BOLD | TEXT_MULTILINES, -s->vmag, &s->obj);
}
...
static int star_render(obj_t *obj, const painter_t *painter_)
{
    ...
    if (!core_get_point_for_mag(star->vmag, &size, &luminance))
        return 0;
    // Unknown B-V -> neutral white (0.43 = table white point), not bluish 0.0.
    bv_to_rgb(isnan(star->bv) ? 0.43 : star->bv, color);
    ...
    int (*f)(const obj_t *obj, void *user, const char *cat, const char *str))
{
    ...
    while (names && *names) {
        if (strncmp(names, "HIP ", 4) == 0) has_hip = true;
        f(obj, user, NULL, names);
        names += strlen(names) + 1;
    }
    // The survey ids don't carry the HIP designation; synthesize it so that
    // designation-based consumers (cultural names for the selection card,
    // search) can match the "HIP n"-keyed skyculture common_names — the
    // label renderer already builds this key itself (star_render_name).
    if (star->hip && !has_hip) {
        snprintf(buf, sizeof(buf), "HIP %d", star->hip);
        f(obj, user, NULL, buf);
    }
    if (star->gaia) {
                          ...
                          double *illuminance)
{
    ...
    for (i = 0; i < tile->nb; i++) {
        ...
        bv_to_rgb(isnan(s->bv) ? 0.43 : s->bv, color);
        points[n] = (point_t) {
            .pos = {p_win[0], p_win[1]},
            .size = size,
            .color = {color[0] * 255, color[1] * 255, color[2] * 255,
