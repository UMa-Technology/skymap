...
struct comet {
    ...
    double      vmag;
    double      pvo[2][4];
    double      last_update_tt; // obs->tt of the cached vmag/pvo (NAN = none).
...
static double date2mjd(int year, int month, double day)
{
    ...
    // eraCal2jd returns < 0 and leaves djm0/djm UNWRITTEN on a bad date (e.g.
    // month 0, which a comet record with no Epoch_* fields produces). Guard so
    // we don't return uninitialized stack values.
    if (eraCal2jd(year, month, iday, &djm0, &djm) < 0)
        return NAN;
...
static int comet_update(comet_t *comet, const observer_t *obs)
{
    ...
    // Position and magnitude depend only on obs->tt; skip the Kepler solve +
    // apparent-place transform when it hasn't advanced (paused/static view,
    // and the perpetual background rescan re-visiting already-computed comets).
    if (comet->last_update_tt == obs->tt) return 0;
    comet->last_update_tt = obs->tt;
...
static int comet_render(obj_t *obj, const painter_t *painter)
{
    ...
    if (*comet->name && (selected || (g_comets->hints_visible &&
                                      ...
                                      hints_mag_offset))) {
        if (selected)
            core_get_selection_label_color(label_color);
        // MPC comet names are English; translate like every other sky
        // label ("10P/Tempel" -> 坦普尔2号彗星). Falls back to the
        // original string when the catalog has no entry.
        labels_add_3d(sys_translate("sky", comet->name),
            FRAME_ICRF, comet->pvo[0], false, size + 4,
...
static int comets_update(obj_t *obj, double dt)
{
    ...
    // Sanity check that search resolves a couple of well-known comets. These
    // are data-dependent (an MPC refresh can drop either object), so warn
    // rather than assert — a data change must never abort the engine.
    obj = core_search("NAME C/1995 O1 (Hale-Bopp)");
    if (!obj || strcmp(obj->klass->id, "mpc_comet") != 0)
        LOG_W("Comet search sanity check failed: Hale-Bopp not found");
    obj = core_search("NAME 1P/Halley");
    if (!obj || strcmp(obj->klass->id, "mpc_comet") != 0)
        LOG_W("Comet search sanity check failed: 1P/Halley not found");
    return 0;
}
