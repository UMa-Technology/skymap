...
typedef struct render_data render_data_t;

/*
 * Type: atmosphere_t
 * Atmosphere module struct.
 */
typedef struct atmosphere atmosphere_t;
...
struct atmosphere {
    ...
    // Cache of the sun/moon-dependent precomputed model (Preetham factors +
    // skybrightness prep).  At real-time speed the inputs move so slowly that
    // the same data stays valid for many seconds; rebuilding it every frame
    // was pure per-frame CPU waste (see the old "XXX: this could be cached").
    struct {
        bool            valid;
        // Inputs the cache was built for.
        double          sun_pos[3], moon_pos[3];
        double          sun_vmag, moon_vmag, turbidity, bortle;
        double          phi, hm;
        int             year, month;
        render_data_t   data;
        // Coarse whole-sky luminance probe, used to skip the render pass
        // entirely when the atmosphere could not add a visible amount of
        // light (night), and to keep feeding the eye-adaptation bookkeeping
        // while skipping.
        double          max_lum, avg_lum;
    } cache;
};
...
/*
 * Coarse whole-sky luminance probe: zenith + a few altitude rings.  The sky
 * model varies smoothly except right next to the sun/moon, where the same
 * cos_grid_angular_step clamp used at render time caps our samples the same
 * way, so the probe max is a faithful bound of what rendering would produce.
 */
static void sample_sky_lum(const render_data_t *d,
                           double *max_lum, double *avg_lum)
{
    static const double alts_deg[] = {2, 10, 30, 60, 90};
    double p[3], az, alt, lum, sum = 0, max = 0;
    int i, j, n = 0, naz;

    for (i = 0; i < (int)ARRAY_SIZE(alts_deg); i++) {
        alt = alts_deg[i] * DD2R;
        naz = alts_deg[i] == 90 ? 1 : 16;
        for (j = 0; j < naz; j++) {
            az = j * 2 * M_PI / naz;
            p[0] = cos(alt) * cos(az);
            p[1] = cos(alt) * sin(az);
...
/*
 * Return the cached render data, rebuilding it only when an input moved
 * beyond thresholds (~0.01 deg of sun/moon motion, i.e. a couple of minutes
 * of real time; every frame during fast time-lapse, same as the old code).
 */
static const render_data_t *get_render_data(
        atmosphere_t *atm, const painter_t *painter,
        const double sun_pos[3], double sun_vmag,
        const double moon_pos[3], double moon_vmag)
{
    const observer_t *obs = painter->obs;
    const double pos_eps = 0.01 * DD2R;
    typeof(atm->cache) *c = &atm->cache;
    int year, month;

    mjd2gcal(obs->utc, &year, &month);
    if (c->valid &&
            vec3_sep(sun_pos, c->sun_pos) < pos_eps &&
            vec3_sep(moon_pos, c->moon_pos) < pos_eps &&
            fabs(sun_vmag - c->sun_vmag) < 0.01 &&
            fabs(moon_vmag - c->moon_vmag) < 0.05 &&
            atm->turbidity == c->turbidity &&
            core->bortle_index == c->bortle &&
            fabs(obs->phi - c->phi) < 1e-5 &&
            fabs(obs->hm - c->hm) < 1.0 &&
            year == c->year && month == c->month)
        return &c->data;

    c->data = prepare_render_data(sun_pos, sun_vmag, moon_pos, moon_vmag,
                                  atm->turbidity, core->bortle_index);
    // This is quite ad-hoc as in reality we are using a HIPS grid
    c->data.cos_grid_angular_step = cos(15. * DD2R);
    prepare_skybrightness(&c->data.skybrightness,
                          painter, sun_pos, moon_pos, moon_vmag);
    sample_sky_lum(&c->data, &c->max_lum, &c->avg_lum);
    vec3_copy(sun_pos, c->sun_pos);
    vec3_copy(moon_pos, c->moon_pos);
    c->sun_vmag = sun_vmag;
    c->moon_vmag = moon_vmag;
    c->turbidity = atm->turbidity;
    c->bortle = core->bortle_index;
    c->phi = obs->phi;
    c->hm = obs->hm;
    c->year = year;
    c->month = month;
...
static int atmosphere_render(obj_t *obj, const painter_t *painter_)
{
    ...
    data = *get_render_data((atmosphere_t*)obj, &painter,
                            sun_pos, sun_vmag, moon_pos, moon_vmag);
    // Per-frame stats accumulated by compute_lum during rendering.
    data.sum_lum = 0;
    data.max_lum = 0;
    data.nb_lum = 0;

    // Night skip: when even the brightest probe of the whole sky maps to
    // less than ~half of an 8-bit display step, the additive pass cannot
    // change any pixel, so skip it entirely (full-screen fill + per-vertex
    // CPU model, every frame).  Keep the adaptation bookkeeping fed from the
    // cached probe so eye adaptation and lwsky_average stay consistent.
    if (tonemapper_map(&core->tonemapper, atm->cache.max_lum) < 1.0 / 512) {
        core->lwsky_average = atm->cache.avg_lum;
        core_report_luminance_in_fov(atm->cache.max_lum, true);
        return 0;
    }
