...
struct label
{
    ...
    bool    shown;        // Geometric visibility settled by labels_render's
                          // first pass (active + de-clutter + occultation).
...
/*
 * Suppress a floating label whose identical text is already carried by a
 * higher-priority label nearby on screen. Several distinct catalog objects can
 * share one proper name (e.g. NGC 2237/2238/2244/... all resolve to "Rosette
 * Nebula"); each renders its own marker + label, and the geometric de-clutter in
 * test_label_overlaps() is text-blind, so the name piled up N times. We keep only
 * the top-priority instance's text. This hides only the duplicated *text*: every
 * object keeps its own clickable area (areas_add_ellipse) and label hit-test, so
 * each component stays individually selectable and still shows the proper name in
 * the info panel.
 *
 * The winner is deterministic (higher priority, ties broken by pointer) so the
 * same node always wins frame to frame — no flicker. The bound is an ON-SKY
 * angular separation, not a screen distance: it is independent of zoom and device
 * pixel ratio, and it matches the data — the components of one named complex sit
 * within a few degrees (Rosette ~0.5°, Taurus Dark Cloud ~4.9°), whereas two
 * genuinely different objects that happen to share a common name (NGC 3115 and
 * NGC 5866 are both "Spindle Galaxy", ~88° apart) are far beyond it and both stay
 * labelled.
 */
#define LABEL_DEDUP_MAX_SEP (6.0 * DD2R)   // rad; same-name labels within = one complex

static bool same_text_suppressed(const label_t *label)
{
    label_t *other;
    double d;

    if (!(label->effects & TEXT_FLOAT)) return false;  // pinned/selected always show
    if (label->frame == -1 || !label->at_inf) return false;  // sky-anchored only
    DL_FOREACH(g_labels->labels, other) {
        if (other == label) continue;
        // Only a label that will actually be drawn suppresses (settled by
        // labels_render's first pass). Checking `active` instead let a winner
        // that was itself hidden by de-clutter or occultation still suppress
        // every duplicate, making the name vanish entirely.
        if (!other->shown) continue;
        if (other->frame != label->frame || !other->at_inf) continue;
        if (strcmp(other->text, label->text) != 0) continue;
        // Only a strictly higher-ranked instance suppresses this one, so exactly
        // one instance of any duplicate cluster survives (no mutual annihilation).
        if (!(other->priority > label->priority ||
              (other->priority == label->priority &&
               (void*)other > (void*)label)))
            continue;
        d = vec3_dot(label->pos, other->pos);       // both normalized (at_inf)
        d = d > 1.0 ? 1.0 : (d < -1.0 ? -1.0 : d);
        if (acos(d) <= LABEL_DEDUP_MAX_SEP) return true;
    }
    return false;
}
...
static int labels_render(obj_t *obj, const painter_t *painter_)
{
    ...
    // Pass 1: settle each label's geometric visibility (active + de-clutter +
    // occultation) into `shown`, independent of same-text suppression. This
    // must be resolved for ALL labels before any suppression decision, so a
    // label that is itself hidden here cannot suppress a same-named sibling.
    DL_FOREACH(g_labels->labels, label) {
        ...
        label->shown = label->fader.target;
    }

    // Pass 2: apply same-text suppression against the settled visibility, then
    // paint.
    DL_FOREACH(g_labels->labels, label) {
        if (g_labels->hidden_obj && label->obj == g_labels->hidden_obj)
            continue;

        if (label->shown && same_text_suppressed(label))
            label->fader.target = false;
