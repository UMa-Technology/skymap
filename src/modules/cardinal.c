...
static int cardinal_render(obj_t *obj, const painter_t *painter)
{
    ...
    // Cardinal marker dot + text (方位文字): #F48123 at 70%.
    double color[4] = {0.957, 0.506, 0.137, 0.7 * c->visible.value};
    double p[4];
    ...
    for (i = 0; i < 8; i++) {
        ...
        _painter.color[3] = color[3];
        _painter.lines.width = 4;
        paint_2d_ellipse(&_painter, NULL, 0, p, VEC(1, 1), NULL);
        // radius pushes the text away from the marker dot (ALIGN_TOP -> down);
        // widened to roughly double the dot<->text gap.
        labels_add_3d(sys_translate("gui", POINTS[i].text), FRAME_OBSERVED,
                      POINTS[i].pos, true, 8.0, size, color, 0,
