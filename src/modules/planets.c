...
struct planet {
    ...
    texture_t   *disk_tex;         // Flat billboard texture (Sun only).
        ...
        double scale, double point_size)
{
    ...
    // Selected solar-system label: #F48123 (white under night mode), same as
    // stars/comets/minorplanets.
    ...
    labels_add_3d(name, FRAME_ICRF, pos,
                  ...
                  selected ? selected_color : label_color, 0, 0,
...
// Fraction of the sun-disk.webp width covered by the photospheric disk (the
// rest is the black margin + prominences that reach just past the limb).
#define SUN_DISK_FRAC 0.978

// Render the Sun as a flat, camera-facing billboard textured with a photo of
// the solar disk (prominences included), sized so the photospheric disk matches
// the Sun's true apparent diameter. paint_texture uses alpha blending, so
// sun-disk.webp is RGBA with the black margins made transparent.
static void planet_render_sun_disk(const planet_t *planet, const double p_win[2],
                                   double ang_diam, double alpha,
                                   const painter_t *painter_)
{
    painter_t painter = *painter_;
    texture_t *tex = planet->disk_tex;
    double size, uv[4][2], color[4] = {1, 1, 1, alpha};
    int i;
    if (!texture_load(tex, NULL)) return; // not downloaded yet
    // texture_set_data() uploads the photo into a power-of-two texture, padding
    // a non-power-of-two image (sun-disk.webp is 3072x3072) into the top-left
    // corner. Sample only the real image sub-rect [0, w/tex_w] x [0, h/tex_h];
    // the default full [0,1] uv would stretch the transparent padding into the
    // quad, both shrinking the disk (~74%) and shifting it off the Sun's centre.
    for (i = 0; i < 4; i++) {
        uv[i][0] = ((i % 2) * tex->w) / (double)tex->tex_w;
        uv[i][1] = ((i / 2) * tex->h) / (double)tex->tex_h;
    }
    size = 2.0 * core_get_point_for_apparent_angle(painter.proj, ang_diam / 2.0)
           / SUN_DISK_FRAC;
    paint_texture(&painter, tex, uv, p_win, size, color, 0);
}
...
static void planet_render(const planet_t *planet, const painter_t *painter_)
{
    ...
    // The Sun never borrows the generic default survey (a moon texture);
    // without its own survey it renders as a glowing point like the stars.
    has_model = planet->hips ?:
                (planet->id == SUN ? NULL : planets->default_hips);
    ...
    if (model_alpha > 0) {
        if (planet->id == SUN && planet->disk_tex) {
            planet_render_sun_disk(planet, p_win, diam * r_scale, model_alpha,
                                   &painter);
        } else {
    ...
    // No extra halo texture for the Sun: its point uses the same glow shader
    // as the stars. The old 200px halo.png quad showed its square edge at
    // wide FOV (the point shrinks with eye adaptation and stops covering it).
}
        ...
        obj_t *obj, const char *url, const char *key)
{
    ...
    // The Sun renders as a flat, camera-facing billboard (a photo of the solar
    // disk, prominences and all) rather than a 3d sphere: a sphere can't show
    // prominences past the limb, and its fixed no-spin frame distorts the disk
    // across dates. Load the RGBA disk photo shipped next to the survey.
    if (p->id == SUN) {
        char tex_url[1024];
        snprintf(tex_url, sizeof(tex_url), "%s/sun-disk.webp", url);
        p->disk_tex = texture_from_url(tex_url, TF_LAZY_LOAD);
    }
    return 0;
}
