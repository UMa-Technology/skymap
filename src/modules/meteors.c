...
static shower_t *create_shower(const json_value *doc)
{
    ...
    vec3_from_sphe(ra * DD2R, dec * DD2R, s->pos);
...
static int meteors_update(obj_t *obj, double dt)
{
    ...
    // A live shooting star is an animation: report it (via core_update's
    // return value) so the render loop keeps producing frames while it
