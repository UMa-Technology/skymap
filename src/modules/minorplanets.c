...
static void load_data(mplanets_t *mplanets, const char *data, int size)
{
    ...
    while (iter_lines(data, size, &line, &len)) {
        ...
        // The 6-bit field spans 0..63 but only 0..10 are defined; a corrupt or
        // future-extended record must not index past the array.
        if (orbit_type >= ARRAY_SIZE(ORBIT_TYPES) || !ORBIT_TYPES[orbit_type])
            orbit_type = 0;
...
static int mplanet_render(obj_t *obj, const painter_t *painter)
{
    ...
    if (*mplanet->name && (selected || (g_mplanets->hints_visible &&
        ...
        // Localize the name (e.g. "Vesta" -> "灶神星"), like the planets do.
        labels_add_3d(sys_translate("sky", mplanet->name),
              FRAME_ICRF, pvo[0], false, size + 4,
