...
static int dss_render(obj_t *obj, const painter_t *painter)
{
    ...
    // Fade the DSS in as the Milky Way texture fades out. milkyway.c fades out
    // over 30..25 deg; the DSS LEADS, fading in over 30..27 deg (hidden >=30 deg,
    // fully shown <=27 deg), so it reaches full while the milky way is still ~40%
    // and the two blend through the whole handoff -> no switching gap/dip.
    ...
    // Not bright enough to draw yet: warm the tiles instead, to kill the blank
    // flash at the Milky Way -> DSS handoff. We start prefetching once the fov
    // is narrow enough to be approaching the handoff (well above the 30 deg
    // fade-in), so the visible tiles are downloaded+decoded by the time DSS
    // becomes visible. Once the layer actually renders, hips_render does the
    // identical traversal itself, so prefetching then would only double the
    // work.
    if (painter2.color[3] < 3.0 / 255) {
        if (core->fov < 40 * DD2R)
            hips_prefetch(dss->hips, painter);
        return 0;
    }
    ...
    // Use the survey-clamped order (max 4 for this survey): an unclamped base
    // lets the pole-case split hit 256 at deep zoom, whose (256+1)^2 vertex
    // grid overflows the uint16 index buffer.
    render_order = hips_get_render_order_clamped(dss->hips, painter);
...
static int dss_add_data_source(obj_t *obj, const char *url, const char *key)
{
    ...
    // The DSS survey is bundled/same-origin, so skip the per-tile download
    // delay: it only adds latency (a visible blank as the layer fades in) with
    // no remote server to protect. See hips_prefetch() call in dss_render().
    hips_set_no_delay(dss->hips, true);
    return 0;
}
