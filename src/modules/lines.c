 ...
 * Night mode colours.
 *
 * Night mode composites a fullscreen #FF6C20 layer over the canvas with
 * 'mix-blend-mode: multiply', so every drawn pixel ends up scaled by
 * (1.0, 0.424, 0.125). The daylight palette is picked for a black sky and
 * does not survive that: the blue azimuthal grid loses 87% of its blue and
 * lands on a near-neutral #16130B, which is both dim and almost hueless
 * against the equally reddened sky.
 *
 * Unlike the DSO/star markers (see core.h nightmode) the grids have no
 * compensation, so each line carries a second palette used when night mode is
 * on. The multiply leaves only the red channel intact, so the remaining
 * degrees of freedom are brightness and a hue axis running orange -> deep red
 * (drawn white -> drawn red). Each line keeps full red and varies G/B, which
 * separates the grids by hue while keeping all of them bright:
 *
 *   drawn white            -> #FF6C20 orange   (observed frame, most used)
 *   drawn (255, 140,  89)  -> #FF3B0B red      (ICRF frame)
 *   drawn (217,  77,  38)  -> #D92105 deep red (JNOW frame)
 *
 * Grid/line pairs sharing a frame keep sharing a hue, exactly as in daylight.
 */
static struct {
    const char  *name;
    const char  *id;
    uint32_t    color;
    uint32_t    night_color; // Used instead of 'color' when nightmode is set.
    ...
    },
    {
        .name       = "Meridian",
        .id         = "meridian",
        // 80 = 50% alpha: the standalone reference lines sit on top of the sky,
        // full opacity made them read as foreground UI. Labels keep 100% —
        // render_label overrides color[3] with the fader value.
        .color      = 0x3c83ff80,
        .night_color = 0xffffff80,
        .frame      = FRAME_OBSERVED,
        .format     = 'n',
        .grid       = false,
    },
    ...
    },
    {
        ...
        .id         = "boundary",
        .color      = 0xffffffff,
        .night_color = 0xffffffff, // Already white: the multiply reddens it.
        .frame      = FRAME_VIEW,
        .grid       = false,
    },
};
...
struct line {
    ...
    bool            grid;       // If true render the whole grid.
    double          color[4];
    double          night_color[4]; // Used instead of 'color' in night mode.
};
...
static int lines_init(obj_t *obj, json_value *args)
{
    ...
    for (i = 0; i < ARRAY_SIZE(LINES); i++) {
        ...
        hex_to_rgba(LINES[i].night_color, line->night_color);
                         ...
                         const painter_t *painter_)
{
    ...
    // 13 + bold: match the sky body labels (stars/dso/planets all render
    // TEXT_BOLD), so the meridian/ecliptic/equator names read as one family
    const double text_size = 13;
    ...
    paint_text_bounds(&painter, buf, p, ALIGN_CENTER | ALIGN_MIDDLE, TEXT_BOLD,
                      text_size, bounds);
    ...
    paint_text(&painter, buf, pos, NULL, ALIGN_CENTER | ALIGN_MIDDLE, TEXT_BOLD,
...
static int line_render(obj_t *obj, const painter_t *painter_)
{
    ...
    // Coordinate grids render as crisp anti-aliased lines, without the glow
    // halo used elsewhere (task #4). This removes the halo and lets render_line
    // narrow the line quad, cutting grid fill by ~3x.
    ...
    // Night mode swaps in the second palette (see the LINES table): the
    // #FF6C20 multiply overlay would otherwise crush the daylight colours into
    // near-hueless dark browns. render_label inherits this RGB, so the labels
    // follow the lines.
    const double *col = core->nightmode ? line->night_color : line->color;
    vec4_copy(col, painter.color);
    painter.color[3] *= line->visible.value;
    ...
    // Emphasize the standalone reference lines (meridian, ecliptic, equator) by
    // drawing them 1.5x wider than the hairline coordinate grids.
    if (!line->grid) painter.lines.width *= 1.5;
