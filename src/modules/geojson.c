    ...
    for (feature = image->features; feature; feature = feature->next) {
        ...
        for (mesh = feature->meshes; mesh; mesh = mesh->next) {
            if (feature->title) {
                ...
                // TEXT_BOLD: match the sky body labels (stars/dso/planets),
                // so jsbridge overlays (time/transit/current/target) read as
                // the same type family as the chart itself
                paint_text(&painter, feature->title, pos, NULL,
                           feature->text_anchor,
                           TEXT_BOLD,
                           feature->text_size > 0 ? feature->text_size :
                                                    FONT_SIZE_BASE,
                           feature->text_rotate);
            }
        }
