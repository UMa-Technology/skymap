...
typedef struct skyculture {
    ...
    // Languages (from index.json "langs_use_native_names") that display the
    // native name untranslated. Whether the *current* language is one of these
    // is decided live against sys_get_lang() (see cult_prefer_native), so a
    // runtime language switch takes effect without reparsing the skyculture.
    char **native_langs;
    int nb_native_langs;
...
static int skyculture_update(obj_t *obj, double dt)
{
    ...
    if (langs_use_native_names) {
        cult->nb_native_langs = langs_use_native_names->u.array.length;
        cult->native_langs = calloc(
            cult->nb_native_langs > 0 ? cult->nb_native_langs : 1,
            sizeof(*cult->native_langs));
        for (i = 0; i < langs_use_native_names->u.array.length; i++) {
            langname = langs_use_native_names->u.array.values[i]->u.string.ptr;
            // strdup: the parsed json doc is freed at the end of this function.
            cult->native_langs[i] = strdup(langname);
...
/*
 * Function: cult_prefer_native
 * Whether the current language (sys_get_lang) is one for which this skyculture
 * shows native names untranslated. Evaluated live so a runtime language change
 * takes effect without reparsing the skyculture.
 */
static bool cult_prefer_native(const skyculture_t *cult)
{
    const char *lang = sys_get_lang();
    int i;
    for (i = 0; i < cult->nb_native_langs; i++) {
        if (strcmp(lang, cult->native_langs[i]) == 0)
            return true;
    }
    return false;
}
                                                   ...
                                                   const char *dsgn))
{
    ...
    // Like skycultures_get_label / add_one_cn: this mode only applies when
    // the entry actually HAS a native name — entries with only an english
    // name (e.g. the ported 星官 common_names) must fall through to the
    // english branch below, or they emit nothing at all.
    if (cult->english_use_native_names && entry->name_native) {
