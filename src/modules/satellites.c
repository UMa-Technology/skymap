...
// The space stations (ISS + the CSS/Tiangong modules) are shown as persistent,
// always-findable markers — like planets — rather than only during a favourable
// pass: their marker + label draw whenever they project on screen, regardless
// of faint magnitude or Earth-shadow eclipse. Other satellites keep the
// realistic magnitude-gated behaviour.
static bool satellite_is_persistent(const satellite_t *sat)
{
    switch (sat->number) {
    case 25544: // ISS (ZARYA)
    case 48274: // CSS Tianhe (the Chinese Space Station / Tiangong)
        return true;
    default:
        return false;
    }
...
// Apparent position (FRAME_ICRF, AU) of a satellite at an arbitrary time, using
// the CURRENT observer — i.e. a snapshot of the orbit as it projects right now.
// Replays satellite_update()'s pipeline at `utc`. Returns false on SGP4 error.
static bool satellite_orbit_point(const satellite_t *sat, const observer_t *obs,
                                  double utc, double out[3])
{
    double pv[2][3];
    if (sgp4(sat->elsetrec, utc, pv[0], pv[1])) return false;
    vec3_mul(1000.0 * DM2AU, pv[0], pv[0]);
    vec3_mul(1000.0 * DM2AU * 60 * 60 * 24, pv[1], pv[1]);
    true_equator_to_j2000(obs, pv, pv);
    position_to_apparent(obs, ORIGIN_GEOCENTRIC, false, pv, pv);
...
// Draw the selected satellite's orbit as a dashed 2px line over one full period.
static void satellite_render_orbit(const satellite_t *sat,
                                   const painter_t *painter_)
{
    const int N = 96; // samples over one orbit; smooth enough for a LEO ellipse.
    // #F48123 at 70% alpha — the shared selection colour, drawn dashed.
    const double orbit_color[4] = {0.95686, 0.50588, 0.13725, 0.70};
    double period, t0, points[N + 1][3];
    painter_t painter = *painter_;
    int i;

    period = sgp4_get_period(sat->elsetrec); // days.
    if (period <= 0) return;
    t0 = painter.obs->utc;
    for (i = 0; i <= N; i++) {
        if (!satellite_orbit_point(sat, painter.obs, t0 + period * i / N,
                                   points[i]))
            return; // SGP4 error mid-orbit: skip the whole track.
    }
    // The orbit precesses (~0.3 deg/period for ISS), so after one Keplerian
    // period the satellite is not exactly back at the start. Snap the last
    // vertex onto the first so the drawn track reads as a clean closed loop.
    vec3_copy(points[0], points[N]);
    vec4_copy(orbit_color, painter.color);
    painter.lines.width = 2.0;
    painter.lines.glow = 0.0;
    painter.lines.dash_length = 12.0; // window pixels.
    painter.lines.dash_ratio = 0.5;   // equal dash / gap.
    paint_linestring(&painter, FRAME_ICRF, N + 1, points);
}
...
static int satellite_render(obj_t *obj, const painter_t *painter_)
{
    ...
    const bool persistent = satellite_is_persistent(sat);
    ...
    // A selected station draws its full-period orbit track (dashed), even when
    // its own dot is currently below the horizon.
    if (selected) satellite_render_orbit(sat, &painter);
    ...
    // Persistent stations skip the faint-magnitude cull so they stay findable.
    if (!persistent && !model_alpha && !selected &&
            vmag > painter.stars_limit_mag && vmag > hints_limit_mag)
        return 0;

    core_get_point_for_mag(vmag, &size, &luminance);
    if (persistent) {
        // Eclipse forces vmag to 17 and below-horizon to 99, which core's
        // skip_point_radius then zeroes to an invisible dot. Floor the marker
        // so these always-on stations keep a visible point.
        size = fmax(size, 3.0);
        luminance = fmax(luminance, 1.0);
    }
    ...
    if (g_satellites->hints_visible &&
            (selected || persistent || vmag <= hints_limit_mag)) {
        vec4_copy(label_color, color);
        // The satellite icon keeps its green tint even when selected (selection
        // is conveyed by the orange label + the orbit track), so ISS matches the
        // CSS modules instead of turning white on select.
    ...
    if (g_satellites->hints_visible &&
        (selected || persistent || vmag <= hints_limit_mag - 1.5)) {
        ...
        if (satellite_get_short_name(sat, selected, buf, sizeof(buf))) {
            // Selected label uses the shared selection colour (#F48123, white
            // under night mode) like stars/DSOs/planets/comets, instead of the
            // plain white that never turned orange in the default view.
            double selected_color[4];
            core_get_selection_label_color(selected_color);
            labels_add_3d(buf, FRAME_ICRF, sat->pvo[0], false, size + 1,
                          FONT_SIZE_BASE - 2, // 13px, same as star/DSO labels.
                          selected ? selected_color : label_color, 0,
                          // Always bold; every engine label also gets a shadow
                          // outline for free via text_shadow_effect. Drop
                          // TEXT_FLOAT when selected so the label pins in place.
                          0, TEXT_BOLD | (selected ? 0 : TEXT_FLOAT), 0, obj);
        }
    }
...
/*
 * Function: satellites_set_tle
 * Update an already-loaded satellite's orbital elements (TLE) IN PLACE.
 *
 * This is the safe way to refresh a satellite from fresh TLE data: it rewrites
 * the SGP4 elset of the EXISTING satellite object, so the object pointer never
 * changes and stays valid in the module's cross-frame render caches
 * (sats->visibles, sats->render_current) and in core->selection. Removing and
 * re-adding a satellite instead would free an object those caches still
 * reference by raw pointer -> use-after-free on the next render frame (which is
 * exactly what happens during a visible pass, when a refresh matters most).
 *
 * The satellite is found by its NORAD catalog number. A NORAD not yet present
 * is left to the frontend to add through the normal createObj path (a pure add
 * is always safe).
 *
 * Called from JS via ccall (see the frontend satellites.js glue module).
 *
 * Returns:
 *   0  on success,
 *  -1  if no loaded satellite has that NORAD number,
 *  -2  if the new TLE could not be parsed (the old elset is kept untouched).
 */
EMSCRIPTEN_KEEPALIVE
int satellites_set_tle(int norad_number, const char *tle1, const char *tle2)
{
    satellites_t *sats = g_satellites;
    satellite_t *sat;
    sgp4_elsetrec_t *elsetrec;
    double startmfe, stopmfe, deltamin;

    if (!sats || !tle1 || !tle2) return -1;
    for (sat = (void*)sats->obj.children; sat; sat = (void*)sat->obj.next) {
        if (sat->number == norad_number) break;
    }
    if (!sat) return -1;

    elsetrec = sgp4_twoline2rv(tle1, tle2, 'c', 'm', 'i',
                               &startmfe, &stopmfe, &deltamin);
    // sgp4_twoline2rv never returns NULL (Vallado's twoline2rv is void); an
    // unusable TLE is signalled by the error flag latched during parsing. Reject
    // it and KEEP the current good elements instead of swapping in a broken orbit.
    if (!elsetrec || sgp4_get_error(elsetrec)) {
        free(elsetrec);
        return -2;
    }
    free(sat->elsetrec);
    sat->elsetrec = elsetrec;
    sat->error = false; // a fresh valid elset revives a previously-errored sat.
    sat->max_brightness = compute_max_brightness(sat->elsetrec, sat->stdmag);
    return 0;
}

/*
 * Function: satellites_has
 * Return 1 if a satellite with the given NORAD number is currently loaded,
 * else 0. The frontend uses this to decide create-vs-skip for the bundled
 * baseline without overwriting a fresher live one — keyed on the NORAD number,
 * consistent with satellites_set_tle.
 */
EMSCRIPTEN_KEEPALIVE
int satellites_has(int norad_number)
{
    satellites_t *sats = g_satellites;
    satellite_t *sat;
    if (!sats) return 0;
    for (sat = (void*)sats->obj.children; sat; sat = (void*)sat->obj.next) {
        if (sat->number == norad_number) return 1;
    }
    return 0;
}
