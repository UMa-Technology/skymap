...
static int milkyway_render(obj_t *obj, const painter_t *painter_)
{
    ...
    // Fade the milky way out across 30..25 deg FOV, handing off to the DSS
    // survey (dss.c fades in over 30..27 deg, leading so it reaches full while
    // the milky way is still partly visible -> no gap). Full milky way above
    // 30 deg, gone below 25 deg.
    visibility = smoothstep(25 * DD2R, 30 * DD2R, core->fov);
