...
/*
 * Function: sgp4_get_period
 * Return the orbital period in days, derived from the mean motion.
 */
double sgp4_get_period(const sgp4_elsetrec_t *satrec);

/*
 * Function: sgp4_get_error
 * Return the SGP4 error code latched in the elset (0 = OK). A freshly parsed
 * elset with a non-zero error came from an unusable TLE.
 */
int sgp4_get_error(const sgp4_elsetrec_t *satrec);
