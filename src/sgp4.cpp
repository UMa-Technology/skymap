...
{
    // no_kozai is the mean motion in rad/min; period = 2*pi/no_kozai minutes.
    const elsetrec *elrec = (const elsetrec*)satrec;
    if (elrec->no_kozai <= 0) return 0;
    return (2.0 * M_PI / elrec->no_kozai) / 1440.0; // minutes -> days.
}

int sgp4_get_error(const sgp4_elsetrec_t *satrec)
{
    return ((const elsetrec*)satrec)->error;
}
