...
static void cls_paint(const painter_t *painter, const double transf_[3][3])
{
    ...
    // Outer circle (was a rect; rects are reserved for the companion app's
    // camera field-of-view marker) with an inner dashed circle.
...
static void bne_paint(const painter_t *painter, const double transf[3][3])
{
    // Circle instead of a rect: rects are reserved for the companion app's
    // camera field-of-view marker.
    paint_2d_ellipse(painter, transf, 0, NULL, NULL, NULL);
}

static void glc_paint(const painter_t *painter, const double transf[3][3])
{
    // Globular cluster: plain circle. The classic cross-in-circle glyph (a
    // horizontal + vertical line through the circle) was dropped per product
    // request — the cross read as clutter at the small on-sky marker size.
    paint_2d_ellipse(painter, transf, 0, NULL, NULL, NULL);
}
