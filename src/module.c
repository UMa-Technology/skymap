...
obj_t *module_add_new(obj_t *module, const char *type, json_value *args)
{
    ...
    // obj_create returns NULL when the object's init fails (e.g. a
    // schema-invalid data row). Bail before module_add dereferences it; the
    // assert that would catch this is compiled out in release builds.
    if (!ret) return NULL;
