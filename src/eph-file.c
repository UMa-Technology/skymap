                          ...
                          int nb_columns, eph_table_column_t *columns)
{
    ...
    for (i = 0; i < n_col; i++) {
        ...
        // For string columns the caller-declared size is its buffer capacity:
        // a file declaring a larger size (mis-built or corrupt tile) would
        // make eph_read_table_row memcpy past the caller's buffer.
        memcpy(&file_size, data + 32 + i * 20, 4);
        if (*type == 's' && columns[j].size > 0 && file_size > columns[j].size) {
            LOG_E("Eph column size %d exceeds declared %d",
                  file_size, columns[j].size);
            return -1;
        }
        columns[j].size = file_size;
        if (columns[j].start < 0 || columns[j].size < 0 ||
                columns[j].start + columns[j].size > *row_size) {
            LOG_E("Eph column outside row");
            return -1;
        }
        // Fix legacy units.
                       ...
                       ...)
{
    ...
    for (i = 0; i < nb_columns; i++) {
        ...
        case 's':
            v.s = va_arg(ap, char*);
            if (got) {
                memcpy(v.s, data + columns[i].start, columns[i].size);
                // Writers reserve the last byte for the NUL; enforce it so a
                // full-width unterminated field can't make strlen run past
                // the buffer.
                if (columns[i].size > 0) v.s[columns[i].size - 1] = '\0';
            } else {
                memset(v.s, 0, columns[i].size);
            }
            break;
        }
    }
