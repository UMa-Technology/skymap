...
// Advance p by an (v)snprintf return value, saturating at the buffer end:
// (v)snprintf returns the would-be length, so advancing by it unchecked walks p
// past end on truncation, turning the next call's size into a huge (negative
// ptrdiff) value.
static char *saturating_advance(char *p, char *end, int n)
{
    if (n < 0) n = 0;
                               ...
                               const char *format, ...)
{
    ...
    va_start(ap, format);
    p = saturating_advance(p, p_end, snprintf(p, p_end - p, "%s/",
                                              hips->service_url));
    p = saturating_advance(p, p_end, vsnprintf(p, p_end - p, format, ap));
    va_end(ap);
                          ...
                          int *nb_tot, int *nb_loaded)
{
    ...
    int flags = HIPS_LOAD_IN_THREAD | (hips->no_delay ? HIPS_NO_DELAY : 0);
                ...
                const double transf[4][4], int split_order)
{
    ...
    render_order = hips_get_render_order_clamped(hips, painter);

    // Can't split less than the rendering order.
    split_order = fmax(split_order, render_order);
    // Cap the per-tile subdivision: the grid indices are uint16, so more than
    // 2^7 splits per tile ((128+1)^2 = 16641 vertices) risks index wrap-around
    // (and absurd allocations) for no visual gain.
    split_order = fmin(split_order, render_order + 7);
...
{
    int render_order, order, pix;
    hips_iterator_t iter;
    uv_map_t map;
    // Same load flags as render_visitor, so warmed tiles are reused as-is.
    int flags = HIPS_LOAD_IN_THREAD | (hips->no_delay ? HIPS_NO_DELAY : 0);

    // Calling hips_is_ready() also kicks off the properties (and allsky) load,
    // so a dormant survey wakes up here instead of on the first paint.
    if (!hips_is_ready(hips)) return;

    render_order = hips_get_render_order_clamped(hips, painter);

    // Breath first traversal of the visible tiles, warming (but not painting)
    // each one at the render order.
    hips_iter_init(&iter);
    while (hips_iter_next(&iter, &order, &pix)) {
        uv_map_init_healpix(&map, order, pix, false, false);
        map.transf = NULL;
        ...
        // Triggers the async download + threaded decode + texture upload.
        hips_get_tile_texture(hips, order, pix, flags, NULL, NULL, NULL);
    }
}
...
{
    int order = hips_get_render_order(hips, painter);
    // Clamp the render order into physically possible range.
    order = clamp(order, hips->order_min, hips->order);
    order = fmin(order, 9); // Hard limit.
    return order;
}
