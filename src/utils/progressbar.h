...
/*
 * Function: progressbar_count_active
 * Return the number of bars still making progress: excludes errored bars
 * (kept forever for display) and bars that reported no progress recently
 * (stalled).  Used by core_update to report loading as animation without
 * ever pinning the render loop above its static frame rate.
 */
int progressbar_count_active(void);
