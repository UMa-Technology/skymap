...
void texture_set_data(texture_t *tex, const void *data, int w, int h, int bpp)
{
    ...
    if (!is_pow2(w) || !is_pow2(h)) {
        ...
        // 填充区复制边缘像素而非留零：glGenerateMipmap 逐层 2x2 平均会把填充色
        // 混进图像边缘，零填充（透明黑）在深层 mip 下表现为锯齿状黑毛边（照片
        // fov 越小、屏幕占比越小时越明显）。采样 UV 始终限制在 [0, w/tex_w] 子
        // 区域内，填充色只影响过滤混合，取边缘同色即无害。
        for (i = 0; i < h; i++)
            for (j = w; j < tex->tex_w; j++)
                memcpy(&buff0[(i * tex->tex_w + j) * bpp],
                       &buff0[(i * tex->tex_w + w - 1) * bpp], bpp);
        for (i = h; i < tex->tex_h; i++)
            memcpy(&buff0[i * tex->tex_w * bpp],
                   &buff0[(h - 1) * tex->tex_w * bpp], tex->tex_w * bpp);
        data = buff0;
    }
    GL(glActiveTexture(GL_TEXTURE0));
    GL(glBindTexture(GL_TEXTURE_2D, tex->id));
    GL(glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR));
    // MIPMAP_LINEAR（三线性）：NEAREST 只在最近单层内取样，缩放跨越 mip 层时
    // 有可见跳变/颗粒；层间线性插值消除跳变，GLES2/WebGL1 原生支持。
    GL(glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER,
            (tex->flags & TF_MIPMAP)? GL_LINEAR_MIPMAP_LINEAR : GL_LINEAR));
