...
int progressbar_count_active(void)
{
    bar_t *bar;
    int n = 0;
    for (bar = g_bars; bar; bar = bar->hh.next) {
        // Errored bars are kept forever (for display) - not activity.
        if (bar->error) continue;
        // No progress reported for a while: stalled/pending, not activity
        // (would otherwise pin the render loop above its static rate).
        if (g_tick > bar->last_update + 60) continue;
        n++;
    }
    return n;
}
