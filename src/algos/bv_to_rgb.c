...
void bv_to_rgb(double bv, double rgb[3])
{
    ...
    // The COLORS table is white-point normalized (max channel forced to 1.0),
    // which washes out star hues (saturation only ~20-48%). Star brightness is
    // carried in the alpha channel, not RGB, so we can push chromaticity here
    // without affecting brightness. S=2.5 = a natural, not cartoonish, boost.
    //
    // L caps the overall level afterwards. The table peaks at pure white
    // (1,1,1) around B-V 0.43 (A-type stars), and points are drawn additively
    // on a black sky, so those come out at the full 255 — the harshest
    // contrast in the whole sky view. Scaling the finished triplet keeps the
    // hue and the relative saturation from S, and only lowers that peak.
    {
        const double S = 2.5;
        const double L = 0.85;
        ...
        int c;
        for (c = 0; c < 3; c++) {
            rgb[c] = gray + (rgb[c] - gray) * S;
            if (rgb[c] < 0.0) rgb[c] = 0.0;
            if (rgb[c] > 1.0) rgb[c] = 1.0;
            rgb[c] *= L;
        }
    }
}
